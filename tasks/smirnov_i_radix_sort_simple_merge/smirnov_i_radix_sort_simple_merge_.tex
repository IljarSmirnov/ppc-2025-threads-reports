...
\begin{itemize}
    ...
    \item Слияние выполняется с помощью задач в \texttt{tbb::task_group}:
        \begin{itemize}
...
\begin{itemize}
    \item Главный процесс (ранг 0) делит входной массив на равные части и распределяет их между процессами с помощью \texttt{MPI_Scatterv}.
    ...
    \item В каждом процессе создаётся второй пул потоков для выполнения функции \texttt{Merging}:
        ...
        \end{itemize}
    \item Локально отсортированные массивы собираются в главный процесс с помощью \texttt{MPI_Gatherv}.
